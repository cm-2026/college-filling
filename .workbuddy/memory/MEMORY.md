...
## 浙江选科添加"技术"科目（2026-04-03）

- **需求**：浙江考生选择科目时，需要增加"技术"选项
- **修改文件**：`index-mysql.html` 第1378-1388行
- **修改内容**：
  - 3+3模式选科根据省份动态显示
  - 浙江：7选3（物理、化学、生物、政治、历史、地理、技术）
  - 其他3+3省份（北京、天津、上海、山东、海南）：6选3（无技术）
- **技术实现**：
  - 通过`region === '浙江'`判断是否显示"技术"科目
  - 动态生成科目标签和提示文字
- **后端兼容性**：
  - 后端3+3模式查询逻辑是动态生成的（server.js 第320-388行）
  - 自动支持任意科目组合，无需修改后端代码
  - 查询会自动生成包含"技术"的所有组合
- **适用范围**：仅浙江地区有"技术"选项
...
## 地区选择下拉列表显示优化（2026-04-03）

- **需求**：地区选择不管什么情况只要点击，就显示下拉列表
- **修改文件**：`index-mysql.html` 第1332-1333行
- **修改内容**：
  - 添加`click`事件监听器，点击时立即显示完整下拉列表
  - 修改`focus`事件监听器，聚焦时显示完整列表（不再显示空字符串）
  - 保留`input`事件监听器，输入时自动过滤匹配的省份
- **技术实现**：
  ```javascript
  input.addEventListener('focus',()=>openDropdown(''));  // 聚焦时显示完整列表
  input.addEventListener('click',()=>openDropdown(''));  // 点击时显示完整列表
  input.addEventListener('input',()=>{openDropdown(input.value);hidden.value='';}); // 输入时过滤
  ```
- **用户体验改进**：点击输入框立即显示所有省份，无需先输入内容

---

## 三科选科要求查询修复（2026-04-03）

- **问题**：数据库中三科要求只有"物理和化学和生物"形式，没有"+"连接数据
- **修改文件**：`server.js`
  - 3+3模式：第325-333行
  - 3+1+2模式：第474-485行
- **修改内容**：
  - 简化三科匹配逻辑，只保留"和"分隔符
  - 移除"+"分隔符支持
  - 匹配格式从12种减少到6种（6种排列组合）
- **技术实现**：
  ```javascript
  // 只生成6种排列组合（物理和化学和生物、物理和生物和化学等）
  for (let i = 0; i < 3; i++) {
    for (let j = 0; j < 3; j++) {
      for (let k = 0; k < 3; k++) {
        if (i !== j && i !== k && j !== k) {
  ...
  ```
- **影响范围**：浙江、山东、天津、海南、上海、北京等3+3省份，约970条专业记录
    * 重置用户密码
...
## admission_plan 数据导入（2026-04-03）

**数据规模**: 839,504条记录（29个省市）

**导入方式**: Python + mysql命令行工具（75.4秒）
...
**技术要点**:
- SQL文件379MB，使用mysql命令行工具导入
- Python脚本处理Windows编码问题（chcp 65001）
- 数据量增加近5倍（16万 → 84万）

**导入文件**: C:/Users/Administrator/Desktop/admission_plan.sql

---

## 云服务器性能优化 - 4GB内存专用（2026-04-04）

**服务器配置**: 2核CPU, 4GB内存, 60GB SSD, 5Mbps带宽

**诊断结果**:
- InnoDB缓冲池: 128MB（严重不足，适合4GB内存的配置应为2GB）
- 最大连接数: 151
- 数据量: 839,504条记录
...
**优化文件**:
- `optimize-for-4gb.bat` - MySQL自动优化脚本（Windows，右键以管理员运行）
- `backend-optimization-4gb.js` - 后端应用层优化代码
- `optimization-guide-4gb.md` - 完整优化指南
- `test-optimization-4gb.js` - 性能测试脚本

**技术要点**:
- 4GB内存服务器建议MySQL使用50%内存（2GB缓冲池）
- 连接池大小应根据内存调整（4GB内存推荐20个连接）
